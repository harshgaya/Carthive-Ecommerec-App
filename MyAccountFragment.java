...
public class MyAccountFragment extends Fragment {
    ...
    public static int MANAGE_ADDRESS = 1;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        shippedIndicator = view.findViewById(R.id.shipped_indiactor);
                ...
                for (MyOrderItemModel orderItemModel : DBqueries.myOrderItemModelList) {
                    if (!orderItemModel.isCancellationRequested()) {
                        if (!orderItemModel.getOrderStatus().equals("Delivered") && orderItemModel.getOrderStatus().equals("Cancelled")) {
                            ...
                            Glide.with(getContext()).load(orderItemModel.getProductImage()).apply(new RequestOptions().placeholder(R.drawable.placeholdericon)).into(currentOrderImage);
                ...
                for (MyOrderItemModel myOrderItemModel : DBqueries.myOrderItemModelList) {
                    if (i < 4) {
                        if (myOrderItemModel.getOrderStatus().equals("Delivered")) {
                            Glide.with(getContext()).load(myOrderItemModel.getProductImage()).apply(new RequestOptions().placeholder(R.drawable.placeholdericon)).into((CircleImageView) recentOrdersContainer.getChildAt(i));
                            i++;
                        }
...
//        });
    ...
    public void onStart() {
        ...
        if (!DBqueries.profile.equals("")) {
            Glide.with(getContext()).load(DBqueries.profile).apply(new RequestOptions().placeholder(R.drawable.boy)).into(profileView);
        }else{
